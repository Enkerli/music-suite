...
| 5b | Serpe Poly **full parity** — **re-flagged as a blocker 2026-07-20** (requested before, still missing from the plugin). Steps 1–3 of 4 ✅ shipped same session: C++ `PolyParser` (lane split/labels/offsets/lcm, conformance-locked, 11/11 match) + **engine voices** (6 lane slots, automatable base lag, per-lane progressive offset, scenes compose) + **plugin UI** (webapp's poly-blocked-in-plugin gate lifted; lane controls sync with the real automatable params; live per-lane playhead) — poly lanes sound AND are controllable in the plugin now, verified locally (full build + ctest + clean esbuild bundle, hand-computed scheduling checks caught and fixed a real float-precision bug) but not yet in a real host/device/WKWebView (SERPE_POLY §8). Known narrow gap: scene-level progressive rotation doesn't yet know about poly lanes. Step 4 (per-lane analysis) remains | L | P3 everywhere Serpe runs |
...
| 6 | GloriArp next arc (docs/GLORIARP_NEXT.md) — slices A–C ✅ *(2026-07-20)*: expression stage (variety/passing tones/pocket/mixed gate/pass+morph — the Keil correction recorded in §2), live loops (per-pass regeneration, tweaks land at the boundary), ProgGenie → Workspace bus handoff, MIDIcurator learn-clip-as-style + takes. **Awaiting by-ear pass (BROWSER_TEST §10/§11); pocket/variety constants tuned by reasoning only — §3b queue for next session.** Remaining: comping role (the EP ask), groove role, style-pack growth via local models, Vane CC ramps, coupled multi-part pocket, realtime MIDI-in "listen" mode | M each | P2 depth |

...
| **Serpe** | ✅ engines **+ package + CLI** | node conformance: `node WebApp/tests/rhythm-conformance.mjs` (134 vectors); C++ `serpe_conformance` console app (CTest + post-build gate); **`@enkerli/upi`** (the promoted engine, 14 vitest) drives `msuite upi` | ✅ **promoted 2026-07-15**: `apps/serpe/engine/{upi,rhythm,analysis,syncopation,mutate}.js` → `@enkerli/upi` (the DOM SVG views stay in `packages/ui/components/rhythm-views.js`); `msuite upi` speaks the full notation language. Progressive/scenes stay stateful/engine-side |

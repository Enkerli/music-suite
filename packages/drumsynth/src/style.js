 ...
 *
 * Pure: takes a style object, returns events and UPI. No file IO and no MIDI
 * parsing, which is what lets the CLI call it directly — learning still lives in
 * tools/drum-style.mjs because it needs the SMF analysers.
 *
...
export function generate(style, { bars = 4, seed = 1, pass = 0, morph = 0, morphHits, morphDynamics } = {}) {
  // Two axes, independent, `morph` setting both — the rhythm can drift while
  // the dynamics hold, or the reverse. GloriArp splits notes/pocket/rests the
  // same way and for the same reason.
  const mHits = morphHits ?? morph;
  const mDyn = morphDynamics ?? morph;
  const fire = morpher(seed, pass, mHits);
  const dyn = morpher(seed, pass, mDyn);
    ...
    for (const v of style.voices) {
      for (const s of v.slots) {
        ...
        const vel = s.velocity
          ? Math.max(1, Math.min(127, Math.round(s.velocity.mean + (dyn() * 2 - 1) * s.velocity.sd)))
          : 100;
        events.push({ bar, slot: s.slot, drum: v.drum, note: v.note, velocity: vel, push: s.push ?? 0 });
      }
    }
  }
  return { style: style.id, bars, seed, pass, morph: { hits: mHits, dynamics: mDyn },
           slotsPerBar, perBeat: style.grid.perBeat, events };
}
